...
function AgentsPage() {
  ...
  const addAgent = useMutation({
    mutationFn: async ({ email, role }: { email: string; role: string }) => {
      // Find user by email via auth admin API (must go through service_role)
      // Fallback: search profiles by joining auth.users — we can't access auth.users directly via client
      // Use RPC or just look for the user in profiles with a text search
      const { data: { users }, error } = await supabase.auth.admin.listUsers();
      if (error) throw new Error("Impossible de rechercher l'utilisateur (droits insuffisants). Invitez l'agent à s'inscrire d'abord.");

      const found = users.find((u) => u.email?.toLowerCase() === email.toLowerCase());
      if (!found) throw new Error("Utilisateur introuvable. Vérifiez qu'il est déjà inscrit sur VigieCity.");
